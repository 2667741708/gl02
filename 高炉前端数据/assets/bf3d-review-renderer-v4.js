...
const REQUIREMENT_ID = "REQ-BF3D-R2U-ISOLATED-WEB-REVIEW-20260720";
...
const EXPECTED_MODEL_BYTES = 4_275_268;
const MATERIAL_GROUP_NAME = "BF3D_V4_MODE_MATERIAL";
const SECTION_GROUP_NAME = "BF3D_V4_MODE_SECTION";
...
function validateReviewAsset(gltf) {
  ...
  if (!root?.isObject3D) {
    throw new EmptyReviewAssetError("V4 审查资产没有可用的 glTF scene");
  }
  ...
  if (!materialGroup || !sectionGroup || root.children.length !== 2) {
    throw new Error(
      `V4 顶层组合同不匹配：material=${materialGroup ? 1 : 0}/1，section=${
        sectionGroup ? 1 : 0
  ...
  if (!materialLogical.length && !sectionLogical.length) {
    throw new EmptyReviewAssetError("V4 审查资产没有逻辑材质或结构对象");
  }
  if (
...
/**
 * 独立、只读的 GL02 V4 材质/结构审查渲染器。
 *
 * 对应需求：REQ-BF3D-R2U-ISOLATED-WEB-REVIEW-20260720
 * 输入：唯一 review.v4 GLB（浏览器内 SHA-256 硬核验）。
...
class BF3DReviewRenderer {
  ...
  async loadInternal() {
    ...
    try {
      ...
      this.setLoadState(
        "ready",
        "V4 审查资产、SHA、PBR 与物理 Section 合同均已通过。",
      );
