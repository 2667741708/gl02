...
const REQUIREMENT_ID = "REQ-BF3D-R2T-ISOLATED-REVIEW-PREVIEW-20260720";
const MODEL_URL = "models/gl02_blast_furnace_review.v3.glb";
const EXPECTED_MODEL_SHA256 =
  "7e4b3b95343103784500aba354a124262ecf593fe89a3f0aa98348692152574b";
const EXPECTED_MODEL_BYTES = 4_380_396;
...
function validateReviewAsset(gltf) {
  ...
  if (!root?.isObject3D) {
    throw new EmptyReviewAssetError("V3 审查资产没有可用的 glTF scene");
  }
  ...
  if (!materialGroup || !sectionGroup || root.children.length !== 2) {
    throw new Error(
      `V3 顶层组合同不匹配：material=${materialGroup ? 1 : 0}/1，section=${
        sectionGroup ? 1 : 0
  ...
  if (!materialLogical.length && !sectionLogical.length) {
    throw new EmptyReviewAssetError("V3 审查资产没有逻辑材质或结构对象");
  }
  if (
...
/**
 * 独立、只读的 GL02 V3 材质/结构审查渲染器。
 *
 * 对应需求：REQ-BF3D-R2T-ISOLATED-REVIEW-PREVIEW-20260720
 * 输入：唯一 review.v3 GLB（浏览器内 SHA-256 硬核验）。
...
class BF3DReviewRenderer {
  ...
  async loadInternal() {
    ...
    try {
      ...
      this.setLoadState(
        "ready",
        "V3 审查资产、SHA、PBR 与物理 Section 合同均已通过。",
      );
