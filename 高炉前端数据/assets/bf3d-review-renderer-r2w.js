...
const AO_STATUS = "AO 未接入 / REF-PENDING";
const INTERNAL_LAYER_BOUNDARY_STATUS =
...
class BF3DReviewRenderer {
  ...
  async loadInternal() {
    ...
    try {
      ...
      this.setLoadState(
        "ready",
        "R2W 已核验 V5 SHA；仅相机候选生效，GLB/PBR 未改。",
      );
  ...
  setMaterialView(view) {
    if (this.loadState !== "ready" || !this.asset) return;
    if (!["exterior", "exterior-detail", "interior"].includes(view)) {
  ...
  applyView({ resetCamera = false } = {}) {
    ...
    if (this.mode === "structural") {
      ...
      note.textContent =
        `查看钢内表面、背衬、热面、耐火层及两类冷却壁；${INTERNAL_LAYER_BOUNDARY_STATUS}。`;
      description.textContent =
        `内部层近景：使用物理 Section 暴露六类内部材质；${INTERNAL_LAYER_BOUNDARY_STATUS}。`;
      this.cameraPreset = "material-interior-closeup";
    } else if (this.materialView === "exterior-detail") {
      title.textContent = "炉腰外表面材质近景";
      note.textContent =
        `聚焦炉腰区；仅改变相机；${AO_STATUS}。`;
      description.textContent =
        `外表面材质近景：炉腰区投影宽度不低于 60%、高度不低于 25%、中心偏移不高于 15%；${AO_STATUS}。`;
      this.cameraPreset = "material-exterior-belly-closeup";
    } else {
      title.textContent = "外表面材质全景";
      note.textContent = `${EXTERIOR_BOUNDARY_MEANING}${AO_STATUS}。`;
      description.textContent =
        `外表面全景：目标投影高度保持 80%–93%；${EXTERIOR_BOUNDARY_MEANING}${AO_STATUS}。`;
      this.cameraPreset = "material-exterior-global";
    }
  ...
  resetCamera() {
    ...
    if (this.mode === "material" && this.materialView === "interior") {
      ...
      this.mode === "material" &&
      this.materialView === "exterior-detail"
    ) {
      this.frameExteriorDetail();
    } else {
      const group =
  ...
  findMaterialZone(zone) {
    const target = this.asset?.materialLogical.find(
      (object) => normalizeName(object.userData?.bf3d_zone) === zone,
    );
    if (!target) {
      throw new EmptyReviewAssetError(`未找到外表面分区：${zone}`);
    }
    return target;
  }
  ...
  projectObjectToCanvas(object) {
    ...
    if (!projectedVertexCount) {
      throw new EmptyReviewAssetError("投影审查目标没有可用顶点");
    }
  ...
  fitProjectionFraction({
    ...
    metrics = this.projectObjectToCanvas(object);
    this.cameraDirection = direction.toArray();
    this.cameraDistanceScale = distance / Math.max(initialDistance, 0.0001);
    this.mobileInteriorPullbackApplied = false;
    return { distance, metrics };
  }

  frameExteriorDetail() {
    const zone = CAMERA_COMPOSITION.exteriorDetailZone;
    const targetObject = this.findMaterialZone(zone);
    const box = new THREE.Box3().setFromObject(targetObject);
    if (box.isEmpty()) {
      throw new EmptyReviewAssetError(`${zone} 外表面近景包围盒为空`);
    }
    const center = box.getCenter(new THREE.Vector3());
    const sphere = box.getBoundingSphere(new THREE.Sphere());
    const direction = new THREE.Vector3(
    ...
    const metrics = fitted.metrics;
    this.projectionAudit = {
      view: "material-exterior-belly-closeup",
      targetZone: normalizeName(targetObject.userData?.bf3d_zone),
      ...
      passed:
        ...
        metrics.heightFraction >=
          CAMERA_COMPOSITION.exteriorDetailHeightMinimum &&
        metrics.centerOffsetFraction <=
          CAMERA_COMPOSITION.exteriorDetailCenterOffsetMaximum,
    };
  }
  ...
  getState() {
    ...
    return {
      ...
      composition: {
        ...
        aoIntegrated: false,
        aoStatus: AO_STATUS,
        internalLayerBoundaryStatus: INTERNAL_LAYER_BOUNDARY_STATUS,
