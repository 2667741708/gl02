...
QA_MCP_MAX_TOOL_ROUNDS = int(os.environ.get("BF_QA_MCP_MAX_TOOL_ROUNDS", "4"))
...
QA_MCP_BRIDGE_SYSTEM_PROMPT = (
    ...
    "variables 必须使用变量目录解析出的标准变量，不得猜测名称；多点查询允许部分点无数据并逐项说明。"
...
def qa_mcp_chart_plan(question: str) -> dict[str, Any] | None:
    ...
    text = str(question or "")
    if not any(term in text for term in ("画", "图", "曲线", "可视化")):
        return None
...
async def qa_mcp_tool_loop_async(
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            deterministic_chart = qa_mcp_chart_plan(last_user_question(working_messages))
            if deterministic_chart and deterministic_chart["tool"] in tool_names:
                name = deterministic_chart["tool"]
                args = deterministic_chart["arguments"]
                ...
                if emit:
                    emit("tool_start", {"tool": name, "arguments": args, "round": 0, "route": "deterministic_chart"})
                try:
            ...
            for round_index in range(max(1, QA_MCP_MAX_TOOL_ROUNDS)):
                response = call_ollama_chat_obj(working_messages, tools=tools)
...
def run_qa_mcp_tool_loop(
    ...
    return asyncio.run(qa_mcp_tool_loop_async(messages, emit=emit, stop_after_tool_round=stop_after_tool_round))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def prepare_qa_chat(
        ...
        with db_connect() as conn:
            ...
            mcp_prefetch = qa_mcp_prefetch(question)
            ...
            use_mcp_tools = qa_mcp_should_use_tools(question, payload, mcp_prefetch)
            hidden_context = {
    ...
    def handle_qa_chat_json(self, payload: dict[str, Any], question: str) -> None:
        try:
            ...
            if prepared.get("use_mcp_tools"):
                tool_result = run_qa_mcp_tool_loop(prepared["messages"])
