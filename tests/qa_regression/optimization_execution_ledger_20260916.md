...
|4|`QAOPT-R04` 多时间窗与历史基线|P1|production_retest_failed|两窗口和历史基线题仍未取得完整证据|实现TimeWindowPlan和两个不重叠查询步骤|
|5|`QAOPT-R05` 报表工作流读取正文|P1|production_retest_failed|最新日报摘要仍停在目录列表|实现list→select→read→summarize依赖链|
