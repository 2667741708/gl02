"""Run SURF-20 R5 full-shell material candidate.

This stage starts from the approved SURF-20 R4 full-shell material candidate,
keeps the R4 coated-steel material contract,
and restores only the user-selected R1 micro-surface strength
...
EXPECTED_INPUT_SHA256 = "9e5c6f645b0cf3d5c14c2922bda403a7810829e33e02223eedb19e0defdbc3f1"
...
DEFAULT_INPUT_BLEND = MODULE_ROOT / "work" / "SURF_20_20260718_R4" / "SURF20_R4_FULL_SHELL_MATERIAL_CANDIDATE.blend"
DEFAULT_OUTPUT_DIR = MODULE_ROOT / "work" / "SURF_20_20260718_R5"
FORMAL_GLB = PROJECT_ROOT / "高炉前端数据" / "models" / "gl02_blast_furnace.glb"
BLEND_NAME = "SURF20_R5_FULL_SHELL_MATERIAL_CANDIDATE.blend"
MATERIAL_NAME = "SURF20_R5_aged_painted_carbon_steel_shared_world"
MAPPING_EMPTY_NAME = "SURF20_R5_SHARED_WORLD_MAPPING_EMPTY"
...
R4_GRAZING_SOURCE = MODULE_ROOT / "work" / "SURF_20_20260718_R4" / "renders" / "SURF20_R4_08_GRAZING_NEAR.png"
...
def update_pipeline_status(output_dir: Path, report: dict[str, Any], command_record: dict[str, Any]) -> None:
    source_status_path = PROJECT_ROOT / "reports" / "pipeline_status.json"
    ...
    payload.setdefault("stages", {})[STAGE] = {
        ...
        "machine_report": relative_display(output_dir / "surf20_r5_machine_report.json"),
        "material_manifest": relative_display(output_dir / "surf20_r5_material_manifest.json"),
        "summary": relative_display(output_dir / "SURF-20_R5_阶段成果总结.md"),
...
def run_parent(args: argparse.Namespace) -> int:
    ...
    for stale in ("surf20_r5_child_exception.txt", "surf20_r5_validate_exception.txt"):
    ...
    write_json(output_dir / "surf20_r5_execution_command.json", command_record)
    ...
    if completed.returncode == 0 and candidate.is_file():
        ...
        validate_exception = output_dir / "surf20_r5_validate_exception.txt"
        validate_report = output_dir / "surf20_r5_reopen_validation.json"
    ...
    report_path = output_dir / "surf20_r5_machine_report.json"
...
def blender_child(args: argparse.Namespace) -> int:
    ...
    def make_diagnostic_cards() -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        cards = {
            "basecolor": {
                **write_png_rgba(texture_dir / "SURF20_R5_diag_basecolor_shared_world_scale.png", base_pixels, res, res),
                "colorspace": "sRGB",
            },
            "roughness": {
                **write_png_rgba(texture_dir / "SURF20_R5_diag_roughness_shared_world_scale.png", rough_pixels, res, res),
                "colorspace": "Non-Color",
            },
            "height": {
                **write_png_rgba(texture_dir / "SURF20_R5_diag_height_soft_orange_peel.png", height_pixels, res, res),
                "colorspace": "Non-Color",
            },
            "metallic": {
                **write_png_rgba(texture_dir / "SURF20_R5_diag_metallic_painted_mask.png", metallic_pixels, res, res),
                "colorspace": "Non-Color",
            },
        }
        stats = {
    ...
    def render_grazing_comparison(r5_grazing_path: Path) -> dict[str, Any]:
        sources = [
            {
                "version": "R1",
            ...
            {
                "version": "R4",
                "path": R4_GRAZING_SOURCE,
                "label": "R4 bump 0.116 / distance 0.058m / normal 0.34",
            },
            {
                "version": "R5",
                "path": r5_grazing_path,
                "label": f"R5 bump {BUMP_STRENGTH:.3f} / distance {BUMP_DISTANCE_METERS:.3f}m / normal {NORMAL_EQUIVALENT_STRENGTH:.2f}",
            },
        ]
        ...
        try:
            ...
            row = {
                **write_png_rgba(comparison_path, out, out_w, out_h),
                ...
                "comparison_contract": "three source renders use the SURF20 *_08_GRAZING_NEAR fixed-view evidence; composition preserves each source image crop and adds only B/D/N parameter labels.",
            }
            return row
        finally:
        ...
        try:
            ...
            white_mat = make_emission_material("SURF20_R5_COMPARISON_TEXT_WHITE", (0.92, 0.95, 0.92, 1.0))
            orange_mat = make_emission_material("SURF20_R5_COMPARISON_TEXT_ORANGE", (1.0, 0.58, 0.22, 1.0))
            panel_width = 4.0
            panel_height = 2.5
            gap = 0.32
            ...
            for x, source in zip(x_positions, sources):
                ...
                mat = make_emission_material(f"SURF20_R5_COMPARISON_{source['version']}_IMAGE", (1, 1, 1, 1), image)
                bpy.ops.mesh.primitive_plane_add(size=1.0, location=(x, 0.0, 0.0), rotation=(0.0, 0.0, 0.0))
                plane = bpy.context.object
                plane.name = f"SURF20_R5_COMPARISON_{source['version']}_PANEL"
                ...
                title_curve = bpy.data.curves.new(f"SURF20_R5_COMPARISON_{source['version']}_TITLE_CURVE", type="FONT")
                title_curve.body = source["version"]
                title_curve.align_x = "CENTER"
                title_curve.align_y = "CENTER"
                title_curve.size = 0.28
                title_obj = bpy.data.objects.new(f"SURF20_R5_COMPARISON_{source['version']}_TITLE", title_curve)
                ...
                label_curve = bpy.data.curves.new(f"SURF20_R5_COMPARISON_{source['version']}_LABEL_CURVE", type="FONT")
                label_curve.body = source["label"]
                label_curve.align_x = "CENTER"
                label_curve.align_y = "CENTER"
                label_curve.size = 0.16
                label_obj = bpy.data.objects.new(f"SURF20_R5_COMPARISON_{source['version']}_LABEL", label_curve)
            ...
            cam_data = bpy.data.cameras.new("SURF20_R5_COMPARISON_CAMERA_DATA")
            cam = bpy.data.objects.new("SURF20_R5_COMPARISON_CAMERA", cam_data)
            ...
            return {
                ...
                "comparison_contract": "three source renders use the SURF20 *_08_GRAZING_NEAR fixed-view evidence; composition preserves each source image crop and only adds labels.",
            }
        finally:
    ...
    add_area_light("SURF20_R5_P40_SOFT_KEY", (center.x - width * 1.25, bbox_min.y - depth * 2.4, center.z + height * 0.72), center, 1050.0, max(width, depth) * 0.62)
    add_area_light("SURF20_R5_P40_COOL_FILL", (center.x + width * 1.2, bbox_min.y - depth * 1.8, center.z + height * 0.55), center, 245.0, max(width, depth) * 1.22)
    add_area_light("SURF20_R5_P40_TOP_SOFTBOX", (center.x, center.y, bbox_max.z + height * 0.85), center, 185.0, max(width, depth) * 1.55)
    grazing_light = add_area_light("SURF20_R5_GRAZING_RAKE_LIGHT", (bbox_min.x - width * 0.18, bbox_min.y - depth * 0.55, center.z + height * 0.35), center, 780.0, 1.4)
    grazing_light.hide_render = True
    grazing_light.hide_viewport = True

    full_scale = full_height / 0.48
    front_cam = make_camera(
        "SURF20_R5_CAM_FRONT_FULL",
        (full_center.x, full_bbox_max.y + max(full_depth * 5.0, full_height * 1.8), full_center.z),
        full_center,
        full_scale,
    )
    three_cam = make_camera(
        "SURF20_R5_CAM_THREE_QUARTER_FULL",
        (
    ...
    for layer in (7, 10, 13, 16):
        ...
        detail_cameras[f"L{layer}_DETAIL"] = make_camera(
            f"SURF20_R5_CAM_L{layer}_DETAIL",
    ...
    renders = {
        "FRONT_FULL": render(render_dir / "SURF20_R5_01_FRONT_FULL.png", front_cam),
        "THREE_QUARTER_FULL": render(render_dir / "SURF20_R5_02_THREE_QUARTER_FULL.png", three_cam),
    }
    ...
    renders.update({
        "L7_DETAIL": render(render_dir / "SURF20_R5_03_L7_DETAIL.png", detail_cameras["L7_DETAIL"]),
        "L10_DETAIL": render(render_dir / "SURF20_R5_04_L10_DETAIL.png", detail_cameras["L10_DETAIL"]),
        "L13_DETAIL": render(render_dir / "SURF20_R5_05_L13_DETAIL.png", detail_cameras["L13_DETAIL"]),
        "L16_DETAIL": render(render_dir / "SURF20_R5_06_L16_DETAIL.png", detail_cameras["L16_DETAIL"]),
        "SEAM_DIAGNOSTIC": render(render_dir / "SURF20_R5_07_SEAM_DIAGNOSTIC.png", seam_cam),
    })
    grazing_light.hide_render = False
    grazing_light.hide_viewport = False
    renders["GRAZING_NEAR"] = render(render_dir / "SURF20_R5_08_GRAZING_NEAR.png", grazing_cam)
    grazing_light.hide_render = True
    grazing_light.hide_viewport = True
    renders["METALLIC_MASK_DIAGNOSTIC"] = {
        **write_png_rgba(
            render_dir / "SURF20_R5_09_METALLIC_MASK_DIAGNOSTIC.png",
            [
                component
    ...
    renders["R1_R4_R5_GRAZING_COMPARISON"] = render_grazing_comparison(Path(renders["GRAZING_NEAR"]["path"]))
    ...
    material_manifest = {
        ...
        "source_baseline": "SURF-20 R4 approved Blender shell-material candidate, SHA-256 locked by this stage input",
        "appearance": {
    ...
    summary = f"""# SURF-20 R5 阶段成果总结

## R5 定位

- R5 是“用户视觉选择的强化粗糙版本”：以 R4 候选为输入，只把微表面恢复到 R1 视觉强度。
- R4 保留为保守备选：R4 的 coated steel 合同继续有效，适合更克制的现场展示。
- 本阶段没有恢复 R1 的 metallic `0.72`，没有替换正式 GLB，没有执行 GLB 导出、Three.js 复验或生产替换。

## R1 / R4 / R5 视觉对比

- R1：掠射下橘皮/细砂和软凹坑明显；本轮仅采用其 B/D/N 微表面视觉强度。
- R4：metallic `0.06`、roughness `0.56-0.82`、shared mapping scale `0.085` 已获保守候选认可，但用户选择更粗糙的读感。
- R5：保留 R4 低金属涂层身份和几何/传感器合同，bump strength `{BUMP_STRENGTH}`、distance `{BUMP_DISTANCE_METERS}m`、normal equivalent `{NORMAL_EQUIVALENT_STRENGTH}`。

## R5 单变量修改

- 输入从 R4 候选继续：`{relative_display(input_blend_path)}`，SHA-256 `{sha256_file(input_blend_path)}`。
- 五个炉段对象 `{", ".join(EXPECTED_PROCESS_ZONES)}` 统一绑定 `{MATERIAL_NAME}`。
- 未修改 metallic 策略、roughness 目标、共享 Object 世界映射比例、五炉段 mesh/matrix、115 传感器、80 测温点、L7-L16、相机取景安全边距或正式 GLB。
- 保留 R4 全景取景口径；仅在全景渲染时临时隐藏侧向检修塔，保存候选前恢复对象可见性。
- 本轮唯一视觉变量：B/D/N 从 R4 `0.116 / 0.058m / 0.34` 恢复为 R1 `0.16 / 0.10m / 0.45`。

## 参数表

| 项 | R1 | R4 | R5 |
|---|---:|---:|---:|
| metallic | 0.72 | 0.06 | `{METALLIC_MASK_VALUE}` |
| roughness target | 0.44-0.78 | 0.56-0.82 | 0.56-0.82 |
| shared mapping scale | 0.085 | 0.085 | 0.085 |
| bump strength | 0.16 | 0.116 | `{BUMP_STRENGTH}` |
| bump distance | 0.10m | 0.058m | `{BUMP_DISTANCE_METERS}m` |
| normal equivalent | 0.45 | 0.34 | `{NORMAL_EQUIVALENT_STRENGTH}` |
...
- 机器报告：`{relative_display(output_dir / "surf20_r5_machine_report.json")}`
- 材质清单：`{relative_display(output_dir / "surf20_r5_material_manifest.json")}`
- 哈希清单：`{relative_display(output_dir / "artifact_sha256.json")}`
- 断言：`{ok_count}/{len(assertions)}` 通过。

## 渲染与 Comparison 证据

- `SURF20_R5_01_FRONT_FULL.png`
- `SURF20_R5_02_THREE_QUARTER_FULL.png`
- `SURF20_R5_03_L7_DETAIL.png`
- `SURF20_R5_04_L10_DETAIL.png`
- `SURF20_R5_05_L13_DETAIL.png`
- `SURF20_R5_06_L16_DETAIL.png`
- `SURF20_R5_07_SEAM_DIAGNOSTIC.png`
- `SURF20_R5_08_GRAZING_NEAR.png`
- `SURF20_R5_09_METALLIC_MASK_DIAGNOSTIC.png`
- `SURF20_R5_10_R1_R4_R5_GRAZING_COMPARISON.png`
- Comparison 源图与 SHA：`{renders["R1_R4_R5_GRAZING_COMPARISON"]["comparison_sources"]}`。

## 已知边界

- 本阶段未做 GLB 导出、PBR 烘焙、glTF Validator 或 Three.js 复验，正式 GLB SHA 保持 `{EXPECTED_FORMAL_GLB_SHA256}`。
- Comparison 使用 R1/R4 既有同名 `*_08_GRAZING_NEAR.png` 与本阶段 R5 掠射图三列合成；合成只加 B/D/N 参数标签，不改变源图裁切。
- 工作区未找到 `specs/avatar_spec.json` 和 `specs/acceptance_checklist.md`；根级 `reports/pipeline_status.json` 已读取，stage-local `reports/pipeline_status.json` 已写入 R5 隔离目录。
- 视觉批准不由本执行智能体授予。
...
"""
    write_json(output_dir / "surf20_r5_material_manifest.json", material_manifest)
    write_json(output_dir / "surf20_r5_machine_report.json", report)
    write_text(output_dir / "SURF-20_R5_阶段成果总结.md", summary)
...
def validate_child(args: argparse.Namespace) -> int:
    ...
    ok_count = sum(1 for row in assertions if row["ok"])
    write_json(
        output_dir / "surf20_r5_reopen_validation.json",
        {
            "schema_version": 1,
            "stage": STAGE,
            "created_at": now_iso(),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.blender_child:
        try:
            ...
            (args.output_dir.resolve() / "surf20_r5_child_exception.txt").write_text(traceback.format_exc(), encoding="utf-8")
            raise
    if args.validate_child:
        try:
            ...
            args.output_dir.resolve().mkdir(parents=True, exist_ok=True)
            (args.output_dir.resolve() / "surf20_r5_validate_exception.txt").write_text(traceback.format_exc(), encoding="utf-8")
            raise
