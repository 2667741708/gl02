"""Run SURF-20 R3 full-shell material candidate.

This stage starts from the locked SURF-20 R2 full-shell candidate .blend,
adjusts only the micro-surface midpoint between R1 too coarse and R2 too flat
...
EXPECTED_INPUT_SHA256 = "bfff1befebb57d30edd342410761105f5b74cfcc67fd8f6232384cae012a23ca"
...
DEFAULT_INPUT_BLEND = MODULE_ROOT / "work" / "SURF_20_20260718_R2" / "SURF20_R2_FULL_SHELL_MATERIAL_CANDIDATE.blend"
DEFAULT_OUTPUT_DIR = MODULE_ROOT / "work" / "SURF_20_20260718_R3"
FORMAL_GLB = PROJECT_ROOT / "高炉前端数据" / "models" / "gl02_blast_furnace.glb"
BLEND_NAME = "SURF20_R3_FULL_SHELL_MATERIAL_CANDIDATE.blend"
MATERIAL_NAME = "SURF20_R3_aged_painted_carbon_steel_shared_world"
MAPPING_EMPTY_NAME = "SURF20_R3_SHARED_WORLD_MAPPING_EMPTY"
METALLIC_MASK_VALUE = 0.06
METALLIC_HIGH_THRESHOLD = 0.85
NORMAL_EQUIVALENT_STRENGTH = 0.29
BUMP_STRENGTH = 0.09
BUMP_DISTANCE_METERS = 0.045
...
def update_pipeline_status(output_dir: Path, report: dict[str, Any], command_record: dict[str, Any]) -> None:
    ...
    payload.setdefault("stages", {})[STAGE] = {
        ...
        "machine_report": relative_display(output_dir / "surf20_r3_machine_report.json"),
        "material_manifest": relative_display(output_dir / "surf20_r3_material_manifest.json"),
        "summary": relative_display(output_dir / "SURF-20_R3_阶段成果总结.md"),
...
def run_parent(args: argparse.Namespace) -> int:
    ...
    for stale in ("surf20_r3_child_exception.txt", "surf20_r3_validate_exception.txt"):
    ...
    write_json(output_dir / "surf20_r3_execution_command.json", command_record)
    ...
    if completed.returncode == 0 and candidate.is_file():
        ...
        validate_exception = output_dir / "surf20_r3_validate_exception.txt"
        validate_report = output_dir / "surf20_r3_reopen_validation.json"
    ...
    report_path = output_dir / "surf20_r3_machine_report.json"
...
def blender_child(args: argparse.Namespace) -> int:
    ...
    def make_diagnostic_cards() -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        cards = {
            "basecolor": {
                **write_png_rgba(texture_dir / "SURF20_R3_diag_basecolor_shared_world_scale.png", base_pixels, res, res),
                "colorspace": "sRGB",
            },
            "roughness": {
                **write_png_rgba(texture_dir / "SURF20_R3_diag_roughness_shared_world_scale.png", rough_pixels, res, res),
                "colorspace": "Non-Color",
            },
            "height": {
                **write_png_rgba(texture_dir / "SURF20_R3_diag_height_soft_orange_peel.png", height_pixels, res, res),
                "colorspace": "Non-Color",
            },
            "metallic": {
                **write_png_rgba(texture_dir / "SURF20_R3_diag_metallic_painted_mask.png", metallic_pixels, res, res),
                "colorspace": "Non-Color",
            },
        }
        stats = {
    ...
    def render_grazing_comparison(r3_grazing_path: Path) -> dict[str, Any]:
        sources = [
            {
                "version": "R1",
            ...
            {
                "version": "R3",
                "path": r3_grazing_path,
                "label": f"R3 bump {BUMP_STRENGTH:.2f} / distance {BUMP_DISTANCE_METERS:.3f}m / normal {NORMAL_EQUIVALENT_STRENGTH:.2f}",
            },
        ]
        ...
        try:
            ...
            comparison_path = render_dir / "SURF20_R3_10_R1_R2_R3_GRAZING_COMPARISON.png"
            row = {
        ...
        try:
            ...
            white_mat = make_emission_material("SURF20_R3_COMPARISON_TEXT_WHITE", (0.92, 0.95, 0.92, 1.0))
            orange_mat = make_emission_material("SURF20_R3_COMPARISON_TEXT_ORANGE", (1.0, 0.58, 0.22, 1.0))
            panel_width = 4.0
            panel_height = 2.5
            gap = 0.32
            x_positions = [-(panel_width + gap), 0.0, panel_width + gap]
            for x, source in zip(x_positions, sources):
                image = bpy.data.images.load(source["path"].as_posix(), check_existing=False)
                created_images.append(image)
                mat = make_emission_material(f"SURF20_R3_COMPARISON_{source['version']}_IMAGE", (1, 1, 1, 1), image)
                bpy.ops.mesh.primitive_plane_add(size=1.0, location=(x, 0.0, 0.0), rotation=(0.0, 0.0, 0.0))
                plane = bpy.context.object
                plane.name = f"SURF20_R3_COMPARISON_{source['version']}_PANEL"
                ...
                title_curve = bpy.data.curves.new(f"SURF20_R3_COMPARISON_{source['version']}_TITLE_CURVE", type="FONT")
                title_curve.body = source["version"]
                title_curve.align_x = "CENTER"
                title_curve.align_y = "CENTER"
                title_curve.size = 0.28
                title_obj = bpy.data.objects.new(f"SURF20_R3_COMPARISON_{source['version']}_TITLE", title_curve)
                ...
                label_curve = bpy.data.curves.new(f"SURF20_R3_COMPARISON_{source['version']}_LABEL_CURVE", type="FONT")
                label_curve.body = source["label"]
                label_curve.align_x = "CENTER"
                label_curve.align_y = "CENTER"
                label_curve.size = 0.16
                label_obj = bpy.data.objects.new(f"SURF20_R3_COMPARISON_{source['version']}_LABEL", label_curve)
            ...
            cam_data = bpy.data.cameras.new("SURF20_R3_COMPARISON_CAMERA_DATA")
            cam = bpy.data.objects.new("SURF20_R3_COMPARISON_CAMERA", cam_data)
    ...
    add_area_light("SURF20_R3_P40_SOFT_KEY", (center.x - width * 1.25, bbox_min.y - depth * 2.4, center.z + height * 0.72), center, 1050.0, max(width, depth) * 0.62)
    add_area_light("SURF20_R3_P40_COOL_FILL", (center.x + width * 1.2, bbox_min.y - depth * 1.8, center.z + height * 0.55), center, 245.0, max(width, depth) * 1.22)
    add_area_light("SURF20_R3_P40_TOP_SOFTBOX", (center.x, center.y, bbox_max.z + height * 0.85), center, 185.0, max(width, depth) * 1.55)
    grazing_light = add_area_light("SURF20_R3_GRAZING_RAKE_LIGHT", (bbox_min.x - width * 0.18, bbox_min.y - depth * 0.55, center.z + height * 0.35), center, 780.0, 1.4)
    grazing_light.hide_render = True
    grazing_light.hide_viewport = True

    full_scale = full_height / 0.48
    front_cam = make_camera(
        "SURF20_R3_CAM_FRONT_FULL",
        (full_center.x, full_bbox_max.y + max(full_depth * 5.0, full_height * 1.8), full_center.z),
        full_center,
        full_scale,
    )
    three_cam = make_camera(
        "SURF20_R3_CAM_THREE_QUARTER_FULL",
        (
    ...
    for layer in (7, 10, 13, 16):
        ...
        detail_cameras[f"L{layer}_DETAIL"] = make_camera(
            f"SURF20_R3_CAM_L{layer}_DETAIL",
    ...
    renders = {
        "FRONT_FULL": render(render_dir / "SURF20_R3_01_FRONT_FULL.png", front_cam),
        "THREE_QUARTER_FULL": render(render_dir / "SURF20_R3_02_THREE_QUARTER_FULL.png", three_cam),
    }
    ...
    renders.update({
        "L7_DETAIL": render(render_dir / "SURF20_R3_03_L7_DETAIL.png", detail_cameras["L7_DETAIL"]),
        "L10_DETAIL": render(render_dir / "SURF20_R3_04_L10_DETAIL.png", detail_cameras["L10_DETAIL"]),
        "L13_DETAIL": render(render_dir / "SURF20_R3_05_L13_DETAIL.png", detail_cameras["L13_DETAIL"]),
        "L16_DETAIL": render(render_dir / "SURF20_R3_06_L16_DETAIL.png", detail_cameras["L16_DETAIL"]),
        "SEAM_DIAGNOSTIC": render(render_dir / "SURF20_R3_07_SEAM_DIAGNOSTIC.png", seam_cam),
    })
    grazing_light.hide_render = False
    grazing_light.hide_viewport = False
    renders["GRAZING_NEAR"] = render(render_dir / "SURF20_R3_08_GRAZING_NEAR.png", grazing_cam)
    grazing_light.hide_render = True
    grazing_light.hide_viewport = True
    renders["METALLIC_MASK_DIAGNOSTIC"] = {
        **write_png_rgba(
            render_dir / "SURF20_R3_09_METALLIC_MASK_DIAGNOSTIC.png",
            [
                component
    ...
    renders["R1_R2_R3_GRAZING_COMPARISON"] = render_grazing_comparison(Path(renders["GRAZING_NEAR"]["path"]))
    ...
    material_manifest = {
        ...
        "source_baseline": "SURF-20 R2 full-shell material candidate, SHA-256 locked by this stage input",
        "appearance": {
...
- R3：只把 R2 微表面强度向 R1 推进约一半，目标是在掠射近景看到密集但克制的微凹凸，正面 L7/L10/L13/L16 仅轻微可见。

## R3 单变量修改

- 输入从 R2 候选继续：`{relative_display(input_blend_path)}`，SHA-256 `{sha256_file(input_blend_path)}`。
...
- 保留 R2 全景取景口径；仅在全景渲染时临时隐藏侧向检修塔，保存候选前恢复对象可见性。
...
| bump strength | 0.16 | 0.06 | `{BUMP_STRENGTH}` |
| bump distance | 0.10m | 0.03m | `{BUMP_DISTANCE_METERS}m` |
| normal equivalent | 0.45 | 0.24 | `{NORMAL_EQUIVALENT_STRENGTH}` |
...
- 机器报告：`{relative_display(output_dir / "surf20_r3_machine_report.json")}`
- 材质清单：`{relative_display(output_dir / "surf20_r3_material_manifest.json")}`
- 哈希清单：`{relative_display(output_dir / "artifact_sha256.json")}`
- 断言：`{ok_count}/{len(assertions)}` 通过。

## 渲染与 Comparison 证据

- `SURF20_R3_01_FRONT_FULL.png`
- `SURF20_R3_02_THREE_QUARTER_FULL.png`
- `SURF20_R3_03_L7_DETAIL.png`
- `SURF20_R3_04_L10_DETAIL.png`
- `SURF20_R3_05_L13_DETAIL.png`
- `SURF20_R3_06_L16_DETAIL.png`
- `SURF20_R3_07_SEAM_DIAGNOSTIC.png`
- `SURF20_R3_08_GRAZING_NEAR.png`
- `SURF20_R3_09_METALLIC_MASK_DIAGNOSTIC.png`
- `SURF20_R3_10_R1_R2_R3_GRAZING_COMPARISON.png`
- Comparison 源图与 SHA：`{renders["R1_R2_R3_GRAZING_COMPARISON"]["comparison_sources"]}`。

## 已知边界

- 本阶段未做 GLB 导出、PBR 烘焙、glTF Validator 或 Three.js 复验，正式 GLB 保持原 SHA。
- Comparison 使用 R1/R2 既有同名 `*_08_GRAZING_NEAR.png` 与本阶段 R3 掠射图三列合成；合成只加参数标签，不改变源图裁切。
...
"""
    write_json(output_dir / "surf20_r3_material_manifest.json", material_manifest)
    write_json(output_dir / "surf20_r3_machine_report.json", report)
    write_text(output_dir / "SURF-20_R3_阶段成果总结.md", summary)
...
def validate_child(args: argparse.Namespace) -> int:
    ...
    ok_count = sum(1 for row in assertions if row["ok"])
    write_json(
        output_dir / "surf20_r3_reopen_validation.json",
        {
            "schema_version": 1,
            "stage": STAGE,
            "created_at": now_iso(),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.blender_child:
        try:
            ...
            (args.output_dir.resolve() / "surf20_r3_child_exception.txt").write_text(traceback.format_exc(), encoding="utf-8")
            raise
    if args.validate_child:
        try:
            ...
            args.output_dir.resolve().mkdir(parents=True, exist_ok=True)
            (args.output_dir.resolve() / "surf20_r3_validate_exception.txt").write_text(traceback.format_exc(), encoding="utf-8")
            raise
