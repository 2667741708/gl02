"""Run SURF-20 R2 full-shell material candidate.

This stage starts from the locked SURF-20 R1 full-shell candidate .blend,
fixes the R1 visual blockers without touching furnace geometry, sensors,
...
EXPECTED_INPUT_SHA256 = "02f0536e5f423f7dddec720f09629b4c6431ed507119177b3e4bb5c4b836fd86"
...
DEFAULT_INPUT_BLEND = MODULE_ROOT / "work" / "SURF_20_20260718_R1" / "SURF20_R1_FULL_SHELL_MATERIAL_CANDIDATE.blend"
DEFAULT_OUTPUT_DIR = MODULE_ROOT / "work" / "SURF_20_20260718_R2"
FORMAL_GLB = PROJECT_ROOT / "高炉前端数据" / "models" / "gl02_blast_furnace.glb"
BLEND_NAME = "SURF20_R2_FULL_SHELL_MATERIAL_CANDIDATE.blend"
MATERIAL_NAME = "SURF20_R2_aged_painted_carbon_steel_shared_world"
MAPPING_EMPTY_NAME = "SURF20_R2_SHARED_WORLD_MAPPING_EMPTY"
METALLIC_MASK_VALUE = 0.06
METALLIC_HIGH_THRESHOLD = 0.85
NORMAL_EQUIVALENT_STRENGTH = 0.24
BUMP_STRENGTH = 0.06
BUMP_DISTANCE_METERS = 0.03
...
def run_parent(args: argparse.Namespace) -> int:
    ...
    for stale in ("surf20_r2_child_exception.txt", "surf20_r2_validate_exception.txt"):
    ...
    write_json(output_dir / "surf20_r2_execution_command.json", command_record)
    ...
    if completed.returncode == 0 and candidate.is_file():
        ...
        validate_exception = output_dir / "surf20_r2_validate_exception.txt"
        validate_report = output_dir / "surf20_r2_reopen_validation.json"
    ...
    report_path = output_dir / "surf20_r2_machine_report.json"
...
def blender_child(args: argparse.Namespace) -> int:
    ...
    def make_diagnostic_cards() -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        cards = {
            "basecolor": {
                **write_png_rgba(texture_dir / "SURF20_R2_diag_basecolor_shared_world_scale.png", base_pixels, res, res),
                "colorspace": "sRGB",
            },
            "roughness": {
                **write_png_rgba(texture_dir / "SURF20_R2_diag_roughness_shared_world_scale.png", rough_pixels, res, res),
                "colorspace": "Non-Color",
            },
            "height": {
                **write_png_rgba(texture_dir / "SURF20_R2_diag_height_soft_orange_peel.png", height_pixels, res, res),
                "colorspace": "Non-Color",
            },
            "metallic": {
                **write_png_rgba(texture_dir / "SURF20_R2_diag_metallic_painted_mask.png", metallic_pixels, res, res),
                "colorspace": "Non-Color",
            },
        }
        stats = {
    ...
    add_area_light("SURF20_R2_P40_SOFT_KEY", (center.x - width * 1.25, bbox_min.y - depth * 2.4, center.z + height * 0.72), center, 1050.0, max(width, depth) * 0.62)
    add_area_light("SURF20_R2_P40_COOL_FILL", (center.x + width * 1.2, bbox_min.y - depth * 1.8, center.z + height * 0.55), center, 245.0, max(width, depth) * 1.22)
    add_area_light("SURF20_R2_P40_TOP_SOFTBOX", (center.x, center.y, bbox_max.z + height * 0.85), center, 185.0, max(width, depth) * 1.55)
    grazing_light = add_area_light("SURF20_R2_GRAZING_RAKE_LIGHT", (bbox_min.x - width * 0.18, bbox_min.y - depth * 0.55, center.z + height * 0.35), center, 780.0, 1.4)
    grazing_light.hide_render = True
    grazing_light.hide_viewport = True

    full_scale = full_height / 0.48
    front_cam = make_camera(
        "SURF20_R2_CAM_FRONT_FULL",
        (full_center.x, full_bbox_max.y + max(full_depth * 5.0, full_height * 1.8), full_center.z),
        full_center,
        full_scale,
    )
    three_cam = make_camera(
        "SURF20_R2_CAM_THREE_QUARTER_FULL",
        (
    ...
    for layer in (7, 10, 13, 16):
        ...
        detail_cameras[f"L{layer}_DETAIL"] = make_camera(
            f"SURF20_R2_CAM_L{layer}_DETAIL",
    ...
    renders = {
        "FRONT_FULL": render(render_dir / "SURF20_R2_01_FRONT_FULL.png", front_cam),
        "THREE_QUARTER_FULL": render(render_dir / "SURF20_R2_02_THREE_QUARTER_FULL.png", three_cam),
    }
    ...
    renders.update({
        "L7_DETAIL": render(render_dir / "SURF20_R2_03_L7_DETAIL.png", detail_cameras["L7_DETAIL"]),
        "L10_DETAIL": render(render_dir / "SURF20_R2_04_L10_DETAIL.png", detail_cameras["L10_DETAIL"]),
        "L13_DETAIL": render(render_dir / "SURF20_R2_05_L13_DETAIL.png", detail_cameras["L13_DETAIL"]),
        "L16_DETAIL": render(render_dir / "SURF20_R2_06_L16_DETAIL.png", detail_cameras["L16_DETAIL"]),
        "SEAM_DIAGNOSTIC": render(render_dir / "SURF20_R2_07_SEAM_DIAGNOSTIC.png", seam_cam),
    })
    grazing_light.hide_render = False
    grazing_light.hide_viewport = False
    renders["GRAZING_NEAR"] = render(render_dir / "SURF20_R2_08_GRAZING_NEAR.png", grazing_cam)
    grazing_light.hide_render = True
    grazing_light.hide_viewport = True
    renders["METALLIC_MASK_DIAGNOSTIC"] = {
        **write_png_rgba(
            render_dir / "SURF20_R2_09_METALLIC_MASK_DIAGNOSTIC.png",
            [
                component
    ...
    assert_row("render_count_at_least_9", len(renders) >= 9, list(renders.keys()))
    ...
    material_manifest = {
        ...
        "source_baseline": "SURF-20 R1 full-shell material candidate, SHA-256 locked by this stage input",
        "appearance": {
    ...
    report = {
        ...
        "material_manifest": relative_display(output_dir / "surf20_r2_material_manifest.json"),
        "renders": renders,
        ...
        "full_view_render_visibility_note": {
            ...
            "r2_change": "Only the side access tower is hidden during the two full-view renders to avoid a non-top vertical tower dominating the image bbox; furnace top gas/upright structures, platforms, supports, and all five shell zones remain visible and candidate object visibility is restored before save.",
        },
    ...
    summary = f"""# SURF-20 R2 阶段成果总结

## R1 阻断项

- `FRONT_FULL` 和 `THREE_QUARTER_FULL` 顶部非背景内容触边 0px，炉顶构件未满足 900 高下至少 72px 的上下边界。
- 近景 bump/法线过强，材质读感接近水泥、砂浆或灰塑料。
- R1 全炉统一 metallic 0.72，不符合“老化喷漆碳钢”主体低金属度身份。
- 远景主体、平台和支撑层级偏暗。

## 实际修改

- 输入从 R1 候选继续，未修改五炉段 mesh/matrix、传感器、L7-L16 或正式 GLB。
- 五个炉段对象 `{", ".join(EXPECTED_PROCESS_ZONES)}` 统一绑定 `{MATERIAL_NAME}`。
- 全景相机改用炉顶构件与炉壳 bbox 取景；仅在全景渲染时临时隐藏侧向检修塔，炉顶煤气构件、平台/支撑和五炉段保持可见，保存候选前恢复对象可见性。
- 细节相机扩大取景，L7/L10/L13/L16 主层 8 点投影全部进入画面安全边界。
- 主体与平台展示照明适度提亮，仍保持深灰绿、哑光、工业厚重。

## Metallic Mask 身份

- 材质身份：老化喷漆碳钢，主体涂层/尘/锈保持低金属度。
- metallic mask 范围：`{METALLIC_MASK_VALUE}` - `{METALLIC_MASK_VALUE}`；均值 `{METALLIC_MASK_VALUE}`；高金属面积比例 `0.0`。
- 处理原因：本轮没有可靠边缘破损/裸钢因果遮罩，按 Visual Bible 4.5 宁可全低金属度，不添加随机裸钢点。

## 材质/映射

- BaseColor：深灰绿涂层，诊断 luma `{texture_statistics["base_luma"]}`。
- Roughness：目标 `0.56-0.82`，诊断 `{texture_statistics["roughness"]}`。
- Normal/Bump：normal equivalent `{NORMAL_EQUIVALENT_STRENGTH}`；bump strength `{BUMP_STRENGTH}`；distance `{BUMP_DISTANCE_METERS}m`。
- 共享映射：所有炉段使用 `TextureCoordinate Object -> {MAPPING_EMPTY_NAME}`，Mapping scale XYZ = `0.085/0.085/0.085`。
...
- 机器报告：`{relative_display(output_dir / "surf20_r2_machine_report.json")}`
- 材质清单：`{relative_display(output_dir / "surf20_r2_material_manifest.json")}`
- 哈希清单：`{relative_display(output_dir / "artifact_sha256.json")}`
- 断言：`{ok_count}/{len(assertions)}` 通过。

## 渲染

- `SURF20_R2_01_FRONT_FULL.png`
- `SURF20_R2_02_THREE_QUARTER_FULL.png`
- `SURF20_R2_03_L7_DETAIL.png`
- `SURF20_R2_04_L10_DETAIL.png`
- `SURF20_R2_05_L13_DETAIL.png`
- `SURF20_R2_06_L16_DETAIL.png`
- `SURF20_R2_07_SEAM_DIAGNOSTIC.png`
- `SURF20_R2_08_GRAZING_NEAR.png`
- `SURF20_R2_09_METALLIC_MASK_DIAGNOSTIC.png`
...
"""
    write_json(output_dir / "surf20_r2_material_manifest.json", material_manifest)
    write_json(output_dir / "surf20_r2_machine_report.json", report)
    write_text(output_dir / "SURF-20_R2_阶段成果总结.md", summary)
...
def validate_child(args: argparse.Namespace) -> int:
    ...
    ok_count = sum(1 for row in assertions if row["ok"])
    write_json(
        output_dir / "surf20_r2_reopen_validation.json",
        {
            "schema_version": 1,
            "stage": STAGE,
            "created_at": now_iso(),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.blender_child:
        try:
            ...
            (args.output_dir.resolve() / "surf20_r2_child_exception.txt").write_text(traceback.format_exc(), encoding="utf-8")
            raise
    if args.validate_child:
        try:
            ...
            args.output_dir.resolve().mkdir(parents=True, exist_ok=True)
            (args.output_dir.resolve() / "surf20_r2_validate_exception.txt").write_text(traceback.format_exc(), encoding="utf-8")
            raise
