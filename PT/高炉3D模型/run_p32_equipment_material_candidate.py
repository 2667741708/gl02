"""Run the Blender P32 equipment-material candidate."""
...
SCRIPT = HERE / "skills" / "bf3d-industrial-materials" / "scripts" / "p32_equipment_material_candidate.py"
...
def main() -> int:
    ...
    output = (args.output_dir or HERE / "work" / f"P32_EQUIPMENT_MATERIAL_{stamp}").resolve()
    ...
    report = output / "p32_equipment_material_candidate.json"
    if report.is_file():
        data = json.loads(report.read_text(encoding="utf-8"))
        print(json.dumps({"status": data.get("status"), "report": str(report), "candidate": data.get("candidate")}, ensure_ascii=False, indent=2))
