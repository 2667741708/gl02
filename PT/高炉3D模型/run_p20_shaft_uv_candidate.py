"""Run the GL02 representative shaft UV candidate in Blender."""
...
SCRIPT = HERE / "skills" / "bf3d-uv-bake" / "scripts" / "p20_shaft_uv_candidate.py"
...
def main() -> int:
    ...
    output = (args.output_dir or HERE / "work" / f"P20_SHAFT_UV_{stamp}").resolve()
    output.mkdir(parents=True, exist_ok=False)
    command = [str(blender), "--background", str(input_blend), "--python", str(SCRIPT), "--", "--source-glb", str(source), "--output-dir", str(output), "--resolution", str(args.resolution)]
    print(json.dumps({"command": command, "output_dir": str(output)}, ensure_ascii=False, indent=2))
    completed = subprocess.run(command, cwd=ROOT, check=False)
    report_path = output / "p20_shaft_uv_candidate.json"
