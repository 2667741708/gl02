...
def main():
    ...
    parser.add_argument("--integrity-module", type=Path)
    ...
    with raw_pg_connect() as connection:
        ...
        execution_gate = None
        if args.integrity_module:
            sys.path.insert(0, str(args.integrity_module.parent))
            import qa_document_integrity
            execution_gate = qa_document_integrity.inspect(doc['full_text'], [dict(row) for row in rows])
