...
$Req = 'REQ-QA-NO-CODE-EVIDENCE-BASELINE-20260915'
...
$Allowed = @('高炉前端数据/智能助手/backend/ollama_proxy_server.py', '高炉前端数据/智能助手/backend/qa_evidence_policy.py')
$StageRoot = [IO.Path]::GetFullPath($StageRoot)
$ExpectedStage = [IO.Path]::GetFullPath('C:\Users\Administrator\AppData\Local\Temp\qa-evidence-20260915-v2')
...
if ($Plan.changes.Count -ne 2) { throw 'Exactly two reviewed targets required' }
...
if (@($Plan.changes.target | Select-Object -Unique).Count -ne 2) { throw 'Duplicate targets' }
...
Assert-Baselines
& $Python -m py_compile (Join-Path $StageRoot 'ollama_proxy_server.py') (Join-Path $StageRoot 'qa_evidence_policy.py')
...
$Backup = Join-Path $Root ('backups/qa-evidence-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
$ResultPath = Join-Path $StageRoot 'deployment-result.json'
$Result = [ordered]@{ ok=$false; requirement_id=$Req; backup=$Backup; rollback_applied=$false; guard_restored=$false }
try {
    ...
    $Result.old_pid=$OldPid
    $Result.new_pid=$NewPid
    $Result.protected_before=$Before
    $Result.protected_after=Protected-Map
    $Result.http=200
    $Result.installed=@($Plan.changes | ForEach-Object { @{target=$_.target;sha256=(Hash $_.target)} })
} catch {
