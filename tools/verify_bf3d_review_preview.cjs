...
const HTML_PATH = path.join(FRONTEND_ROOT, "bf3d_review.server.html");
...
const SERVER_PATH = path.join(ROOT, "tools", "serve_bf3d_review.py");
...
const EXPECTED_REVIEW_BYTES = 4_380_396;
...
function buildGlbContract() {
  ...
  const materialGroup = nodes.find(
    (node) => node.name === "BF3D_V3_MODE_MATERIAL",
  );
  const sectionGroup = nodes.find(
    (node) => node.name === "BF3D_V3_MODE_SECTION",
  );
  ...
  const checks = [
    ...
    makeCheck(
      ...
      renderer.includes(
        ...
        !renderer.includes("material_review.v3.glb") &&
        !renderer.includes("structural_review.v3.glb"),
    ),
    makeCheck(
...
async function runViewport(browser, engine, viewport, baseUrl, stress) {
  ...
  try {
    ...
    const reviewRequests = requestPaths.filter(
      (pathname) =>
        pathname === "/models/gl02_blast_furnace_review.v3.glb",
    );
