...
const REPORT_PATH = path.join(
  STAGE_ROOT,
  "reports",
  "bf3d_review_r2w_preview_report.json",
);
const HTML_PATH = path.join(FRONTEND_ROOT, "bf3d_review_r2w.server.html");
...
const SERVER_PATH = path.join(ROOT, "tools", "serve_bf3d_review_r2w.py");
...
const V5_HTML_PATH = path.join(FRONTEND_ROOT, "bf3d_review_v5.server.html");
const V5_RENDERER_PATH = path.join(
  FRONTEND_ROOT,
  "assets",
  "bf3d-review-renderer-v5.js",
);
const V5_SERVER_PATH = path.join(ROOT, "tools", "serve_bf3d_review_v5.py");
const V5_VERIFIER_PATH = path.join(
  ROOT,
  "tools",
  "verify_bf3d_review_v5_preview.cjs",
);
...
const EXPECTED_REVIEW_BYTES = 4_663_220;
...
const EXPECTED_V5_SOURCE_SHA256 = Object.freeze({
  html: "bb0ddf0acf79f7ec81652b047e6295d90322fa15461f08d90473833ff5878565",
  renderer:
    "9d01b31482abc7c09146f657ba402457d354890f74f76501ec45dcf7e50b589d",
  server:
    "69adc5ed12b3e41767ce85a34da395aee335b523861116099339faae8efbb855",
  verifier:
    "e73c78753d6a0e86fb31a469bf007980a22e6585b0613e9f886adb2f4a71f9c9",
});
...
function buildGlbContract() {
  ...
  const materialGroup = nodes.find(
    (node) => node.name === "BF3D_V5_MODE_MATERIAL",
  );
  const sectionGroup = nodes.find(
    (node) => node.name === "BF3D_V5_MODE_SECTION",
  );
  ...
  const checks = [
    ...
    makeCheck(
      ...
      {
        interpretation:
          "V5 十个物理 Section 封口均位于同一受控切平面；跨对象正面积重叠已由 R2U 几何验证器独立证明为 0。",
  ...
  const checks = [
    ...
    makeCheck(
      ...
      renderer.includes(
        ...
        !renderer.includes("material_review.v5.glb") &&
        !renderer.includes("structural_review.v5.glb"),
    ),
    makeCheck(
    ...
    makeCheck(
      "two_main_modes_and_four_review_views",
      [
    ...
    makeCheck(
      "r2w_camera_only_readability_contract",
      [
        'exteriorDetailZone: "BELLY"',
        "projectObjectToCanvas",
        "fitProjectionFraction",
        "material-exterior-belly-closeup",
        "materialOrGeometryModified: false",
        "lightingOrColorPathModified: false",
      ].every((token) => renderer.includes(token)) &&
        html.includes("仅新增确定性相机近景") &&
        html.includes("不改灯光、曝光、环境、GLB 或材质"),
    ),
    makeCheck(
      "ao_and_internal_interface_disclosure",
      html.includes("AO 未接入 / REF-PENDING") &&
        html.includes("内部层边界 REF-PENDING，非数据竖线") &&
        renderer.includes('const AO_STATUS = "AO 未接入 / REF-PENDING"') &&
        renderer.includes("INTERNAL_LAYER_BOUNDARY_STATUS"),
    ),
    makeCheck(
      "projection_threshold_contract",
      renderer.includes("exteriorGlobalHeightMinimum: 0.8") &&
        renderer.includes("exteriorGlobalHeightMaximum: 0.93") &&
        renderer.includes("exteriorDetailWidthMinimum: 0.6") &&
        renderer.includes("exteriorDetailHeightMinimum: 0.25") &&
        renderer.includes("exteriorDetailCenterOffsetMaximum: 0.15"),
    ),
    makeCheck(
    ...
    makeCheck(
      ...
      verifier.includes(
        ...
        verifier.includes("bf3d_review_r2w_stability_run_") &&
...
function snapshotProtectedFiles() {
  return {
    ...
    v5_html: inspectFile(V5_HTML_PATH, {
      sha256: EXPECTED_V5_SOURCE_SHA256.html,
    }),
    v5_renderer: inspectFile(V5_RENDERER_PATH, {
      sha256: EXPECTED_V5_SOURCE_SHA256.renderer,
    }),
    v5_server: inspectFile(V5_SERVER_PATH, {
      sha256: EXPECTED_V5_SOURCE_SHA256.server,
    }),
    v5_verifier: inspectFile(V5_VERIFIER_PATH, {
      sha256: EXPECTED_V5_SOURCE_SHA256.verifier,
    }),
  };
}
...
async function runViewport(browser, engine, viewport, baseUrl, stress) {
  ...
  try {
    ...
    checks.push(
      makeCheck(
        ...
        exterior.mode === "material" &&
          ...
          exterior.cameraPreset === "material-exterior-global" &&
          exterior.objects.visibleMaterialLogical === 5 &&
          exterior.objects.visibleSectionLogical === 0 &&
          exterior.composition.projectionAudit?.passed === true &&
          exterior.composition.projectionAudit.metrics.heightFraction >= 0.8 &&
          exterior.composition.projectionAudit.metrics.heightFraction <= 0.93,
        exterior,
      ),
    );
    checks.push(
      makeCheck(
        ...
        exterior.composition.exteriorDataRingOrLeader === false &&
          ...
          exterior.composition.aoIntegrated === false &&
          exterior.composition.aoStatus.includes("REF-PENDING") &&
          exterior.composition.materialOrGeometryModified === false &&
          exterior.composition.lightingOrColorPathModified === false &&
          exteriorUi.note.includes("不是数据圆环、轮廓或引线") &&
          exteriorUi.description.includes("R2J 五区实体交界") &&
          exteriorUi.description.includes("80%–93%"),
        {
    ...
    screenshots.push(
      await captureMode(page, engine, width, height, "material-exterior-global"),
    );

    await page.click('[data-material-view-button="exterior-detail"]');
    await page.waitForFunction(
      () => document.body.dataset.materialView === "exterior-detail",
    );
    const exteriorDetail = await page.evaluate(() =>
      globalThis.BF3D_REVIEW_PREVIEW.getAuditSnapshot(),
    );
    const exteriorDetailUi = await page.evaluate(() => ({
      note: document.querySelector("#visible-set-note")?.textContent || "",
      description:
        document.querySelector("#active-view-description")?.textContent || "",
    }));
    const detailProjection = exteriorDetail.composition.projectionAudit;
    checks.push(
      makeCheck(
        "material_exterior_belly_closeup_projection",
        exteriorDetail.mode === "material" &&
          exteriorDetail.materialView === "exterior-detail" &&
          exteriorDetail.cameraPreset ===
            "material-exterior-belly-closeup" &&
          exteriorDetail.objects.visibleMaterialLogical === 5 &&
          exteriorDetail.objects.visibleSectionLogical === 0 &&
          detailProjection?.targetZone === "BELLY" &&
          detailProjection?.cameraOnly === true &&
          detailProjection?.passed === true &&
          detailProjection.metrics.widthFraction >= 0.6 &&
          detailProjection.metrics.heightFraction >= 0.25 &&
          detailProjection.metrics.centerOffsetFraction <= 0.15 &&
          exteriorDetailUi.note.includes("聚焦炉腰区") &&
          !exteriorDetailUi.note.includes("bf3d_zone") &&
          exteriorDetailUi.description.includes("炉腰区投影") &&
          !exteriorDetailUi.description.includes("BELLY") &&
          exteriorDetailUi.description.includes("AO 未接入 / REF-PENDING"),
        {
          state: exteriorDetail,
          ui: exteriorDetailUi,
        },
      ),
    );
    checks.push(
      makeCheck(
        "pbr_unchanged_after_exterior_detail",
        exteriorDetail.pbr.checked === true &&
          exteriorDetail.pbr.mutationCount === 0 &&
          exteriorDetail.composition.materialOrGeometryModified === false &&
          exteriorDetail.composition.lightingOrColorPathModified === false,
        exteriorDetail.pbr,
      ),
    );
    screenshots.push(
    ...
    checks.push(
      makeCheck(
        ...
        interior.mode === "material" &&
          ...
          interior.composition.internalLayerBoundaryStatus.includes(
            "REF-PENDING",
          ) &&
          interiorUi.note.includes(
    ...
    if (stress) {
      ...
      const materialViews = ["exterior", "exterior-detail", "interior"];
      for (let index = 0; index < 12; index += 1) {
        await page.click('[data-mode-button="material"]');
        await page.click(
          `[data-material-view-button="${materialViews[index % 3]}"]`,
        );
        await page.click('[data-mode-button="structural"]');
      }
    ...
    const reviewRequests = requestPaths.filter(
      (pathname) =>
        pathname === "/models/gl02_blast_furnace_review.v5.glb",
    );
...
function stabilityLogPath(iteration) {
  const name = REPRESENTATIVE_ONLY
    ? "bf3d_review_r2w_representative_run.json"
    : `bf3d_review_r2w_stability_run_${iteration}.json`;
...
function buildStabilityIteration(iteration, results, startedAt, completedAt) {
  ...
  const passed =
    ...
    screenshotCount === requiredRuns * 4;
  const record = {
...
async function main() {
  ...
  const report = {
    ...
    required_screenshots_per_run: 4,
    screenshot_count: 0,
    ...
    limitations: [
      ...
      "AO is not integrated; AO remains REF-PENDING.",
      "The internal layer boundary remains REF-PENDING and is not a data line.",
