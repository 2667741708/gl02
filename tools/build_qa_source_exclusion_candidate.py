"""Freeze inherited source prohibitions over the verified V42 closure."""
import ast
import argparse
import hashlib
import json
...
MODULES = ('qa_task_plan.py', 'qa_document_compound.py', 'qa_document_knowledge.py', 'qa_history_compound.py')


def main(revision='r1'):
    assert revision in {'r1', 'r2', 'r3', 'r4', 'r5', 'r6'}
    target = ROOT / '.codex_runtime/qa-routing-v43' / ('candidate-' + revision)
    sha = lambda raw: hashlib.sha256(raw).hexdigest()
    evidence = json.loads((ROOT / 'tests/qa_regression/source_concept_scope_20260917.json').read_bytes())
    ...
    for name in MODULES:
        payloads[name] = (ROOT / '高炉前端数据/智能助手/backend' / name).read_bytes()
    for name, raw in payloads.items():
    ...
    metadata = {'schema': 'bf.qa.private-source-exclusion-candidate.v1', 'candidate': 'v43-' + revision,
      ...
      'inherited_v42_files_byte_identical': inherited, 'changed_modules': list(MODULES),
    ...
    print(json.dumps({'ok': True, 'candidate': metadata['candidate'], 'files': 15, 'inherited': 11,
      'manifest_sha256': sha(raw), 'production_sealed': False, 'model_operations': 0}))


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--revision', choices=('r1', 'r2', 'r3', 'r4', 'r5', 'r6'), default='r1')
