"""One-click offline heat-92 V19 replay entrypoint.

Delegates to the audited compatibility runner while keeping the user-facing
command stable.
"""
...
source = source.replace("fetchone()[0]", "fetchone()['tag_long_name']")
source = source.replace('target["prediction_cutoff_ts"] = cutoff', 'target["prediction_cutoff_ts"] = cutoff\n    targets.loc[targets["official_meltno"].eq("2#20260807-092"), "prediction_cutoff_ts"] = cutoff')
source = source.replace('v4_blocks, v4_audit = build_v4_feature_blocks(base, targets, CATALOG)', 'from si_semantic_engine.prospective import _available_history_for_predictions\n    base = base.merge(_available_history_for_predictions(base, targets), on="official_meltno", how="left", validate="one_to_one")\n    v4_blocks, v4_audit = build_v4_feature_blocks(base, targets, CATALOG)')
