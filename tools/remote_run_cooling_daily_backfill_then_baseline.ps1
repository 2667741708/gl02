...
$start=(Get-Date).Date.AddDays(-35)
$end=(Get-Date).Date
$state=[ordered]@{schema='ops.cooling-daily-backfill.v1';started_at=Get-Date;window_start=$start;window_end_exclusive=$end;current_day=$null;completed_days=@();failed_days=@();backfill_complete=$false;baseline_complete=$false;baseline_completed_at=$null;updated_at=Get-Date;error=$null}
...
try{
  ...
  Save-State
  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $baseline 2>&1|Out-File -LiteralPath $log -Encoding UTF8 -Append
  if($LASTEXITCODE-ne0){throw ('Baseline rebuild failed with exit code '+$LASTEXITCODE)}
