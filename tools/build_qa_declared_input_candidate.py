"""Freeze declared-input routing over the verified V39 closure, without remote calls."""
import ast
import hashlib
import json
...
TARGET = ROOT / '.codex_runtime/qa-routing-v40/candidate-r2'
MODULE = 'qa_task_plan.py'


def main():
    sha = lambda raw: hashlib.sha256(raw).hexdigest()
    evidence = json.loads((ROOT / 'tests/qa_regression/user_data_source_scope_20260917.json').read_bytes())
    ...
    metadata = {'schema': 'bf.qa.private-declared-input-candidate.v1', 'candidate': 'v40-r2',
      ...
      'inherited_v39_files_byte_identical': inherited, 'changed_module': MODULE,
