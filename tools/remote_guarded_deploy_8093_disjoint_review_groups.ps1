...
$stage = 'C:\Users\Administrator\AppData\Local\Temp\abc33_disjoint_review_groups'
...
$duplicateAudit = Join-Path $stage 'audit_abc33_review_group_duplicates.py'
$python = 'C:\Program Files\Python311\python.exe'
$version = 'abc33-20260810-disjoint-review-groups-r1'
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$backup = Join-Path $root "backups\abc33_disjoint_review_groups_$stamp"
...
foreach ($path in $asset, $reviewBackend, $page8093, $page8094, $stagedAsset, $stagedBackend, $labelAudit, $duplicateAudit, $python) {
...
foreach ($marker in 'function disjointReviewGroups', 'seen.has(key)', '其余炉壳温度点位', '其余冷却系统点位') {
    if (-not $stagedContent.Contains($marker)) { throw "staged asset missing marker: $marker" }
}
$stagedBackendContent = Get-Content -LiteralPath $stagedBackend -Raw -Encoding UTF8
foreach ($marker in 'def _group_metrics', 'main_names', 'remaining') {
    if (-not $stagedBackendContent.Contains($marker)) { throw "staged backend missing marker: $marker" }
}
& $python -m py_compile $stagedBackend $labelAudit $duplicateAudit
...
$duplicateText = (& $python $duplicateAudit --base-url 'http://127.0.0.1:8093' --require-clean | Out-String)
if ($LASTEXITCODE -ne 0) { throw 'ABC33 production duplicate audit command failed' }
$labelResult = $labelText | ConvertFrom-Json
$duplicateResult = $duplicateText | ConvertFrom-Json
...
if ([int]$labelResult.rule_count -ne 33 -or [int]$labelResult.detail_error_count -ne 0 -or [int]$labelResult.issue_metric_count -ne 0) {
    throw 'ABC33 production label audit did not remain clean'
}
if ([int]$duplicateResult.rule_count -ne 33 -or [int]$duplicateResult.detail_error_count -ne 0 -or [int]$duplicateResult.affected_rule_count -ne 0) {
    throw 'ABC33 production review groups are not disjoint'
}
...
[ordered]@{
    ...
    audit_rule_count = [int]$duplicateResult.rule_count
    audit_metric_occurrence_count = [int]$duplicateResult.metric_occurrence_count
    audit_affected_rule_count = [int]$duplicateResult.affected_rule_count
    audit_duplicate_variable_count = [int]$duplicateResult.duplicate_variable_count
    audit_duplicate_occurrence_count = [int]$duplicateResult.duplicate_occurrence_count
