...
$CameraPayload = 'C:\Users\Administrator\AppData\Local\Temp\bf3d-surface-camera-guard-8093.js'
$DeployScript = 'C:\Users\Administrator\AppData\Local\Temp\remote_deploy_8093_initial_camera_framing.py'
...
foreach ($required in @($manager, $config, $python, $CameraPayload, $DeployScript)) {
...
try {
    ...
    $deployOutput = @(& $python $DeployScript --root $Root --camera-payload $CameraPayload 2>&1)
    if ($LASTEXITCODE -ne 0) {
        throw "8093 camera deployment failed: $($deployOutput -join [Environment]::NewLine)"
    }
}
finally {
...
$page = Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:8093/?t=initial-framing-20260802#overview' -TimeoutSec 5
$asset = Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:8093/assets/bf3d-surface-camera-guard-8093.js?v=20260802-overview-closer-r1' -TimeoutSec 5
$cameraHashAfterRestart = (Get-FileHash -LiteralPath (Join-Path $Root '高炉前端数据\assets\bf3d-surface-camera-guard-8093.js') -Algorithm SHA256).Hash
$result = [ordered]@{
    schema = 'ops.8093.guard-pause-deploy-resume.v1'
    requirement_id = 'REQ-BF3D-8093-INITIAL-FRAME-CLOSER-20260802'
    ...
    page_linked = $page.Content.Contains('bf3d-surface-camera-guard-8093.js?v=20260802-overview-closer-r1')
    camera_schema_v4 = $asset.Content.Contains('bf3d.camera.overview-only.8093.v4')
    fit_margin_1 = $asset.Content.Contains('OVERVIEW_FIT_MARGIN = 1.0')
    orbit_margin_112 = $asset.Content.Contains('OVERVIEW_ORBIT_MARGIN = 1.12')
    min_distance_guard_preserved = $asset.Content.Contains('viewer.controls.minDistance = fullOrbitRadius')
    camera_hash_after_guard_restart = $cameraHashAfterRestart
