#!/usr/bin/env python3
"""Serve the isolated GL02 V4 material/structure review page.
...
EXPECTED_REVIEW_BYTES = 4_275_268
LOOPBACK_HOSTS = {"127.0.0.1", "localhost"}

WORKSPACE_ROOT = Path(__file__).resolve().parents[1]
FRONTEND_ROOT = WORKSPACE_ROOT / "高炉前端数据"
REVIEW_ASSET = FRONTEND_ROOT / "models" / "gl02_blast_furnace_review.v4.glb"

ROUTES = {
    "/bf3d_review_v4.server.html": FRONTEND_ROOT / "bf3d_review_v4.server.html",
    "/assets/bf3d-review-renderer-v4.js": (
        FRONTEND_ROOT / "assets" / "bf3d-review-renderer-v4.js"
    ),
    "/assets/bf3d-review-renderer.css": (
        FRONTEND_ROOT / "assets" / "bf3d-review-renderer.css"
    ),
    "/models/gl02_blast_furnace_review.v4.glb": REVIEW_ASSET,
...
def validate_contract() -> dict[str, object]:
    ...
    return {
        ...
        "document": "高炉前端数据/bf3d_review_v4.server.html",
        "review_asset": {
            "path": "高炉前端数据/models/gl02_blast_furnace_review.v4.glb",
            "bytes": actual_bytes,
...
class ReviewRequestHandler(BaseHTTPRequestHandler):
    ...
    def _serve(self, *, send_body: bool) -> None:
        ...
        if pathname == "/":
            ...
            self.send_header("Location", "/bf3d_review_v4.server.html")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="启动 GL02 V4 独立、只读、SHA 锁定的材质/结构审查页。",
    ...
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="只核验 allow-list 与 review.v4 SHA，不启动服务。",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    ready = {
        **contract,
        ...
        "host": actual_host,
        "port": actual_port,
        "url": f"http://{actual_host}:{actual_port}/bf3d_review_v4.server.html",
    }
