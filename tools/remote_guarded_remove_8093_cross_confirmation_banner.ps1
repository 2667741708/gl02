...
$stage = 'C:\Users\Administrator\AppData\Local\Temp\remove_8093_cross_confirmation'
...
$version = 'abc33-20260810-hide-cross-confirmation-8093-r1'
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$backup = Join-Path $root "backups\remove_8093_cross_confirmation_$stamp"
...
if (-not $stagedContent.Contains("window.location.port!=='8093'&&d.event_confirmation_summary")) {
    throw 'staged asset does not contain the 8093 render gate'
}
...
if ($assetResponse.StatusCode -ne 200 -or -not $assetResponse.Content.Contains("window.location.port!=='8093'&&d.event_confirmation_summary")) {
    throw '8093 asset render-gate verification failed'
}
