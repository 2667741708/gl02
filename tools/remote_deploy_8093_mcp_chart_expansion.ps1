...
$stageBackend = Join-Path $root 'logs\ollama_proxy_server.mcp_chart_expansion_20260715.staged.py'
$stageMcp = Join-Path $root 'logs\bf_data_mcp_server.mcp_chart_expansion_20260715.staged.py'
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$backendBackup = "$backend.bak_mcp_chart_expansion_$stamp"
$mcpBackup = "$mcp.bak_mcp_chart_expansion_$stamp"
$deployed = $false
...
foreach ($marker in @('plot_gl02_trends', 'plot_gl02_analysis', 'correlation_heatmap', 'MCP数据图')) {
...
foreach ($marker in @('def normalize_chart_type', 'def render_analysis_chart', 'def plot_gl02_analysis', 'small_multiples')) {
