...
const output = path.resolve(
  argument(
    "--output",
    path.join(path.dirname(assetPath), "maintenance_access_glb_reopen_check.json"),
  ),
);
...
try {
  ...
  const counts = await page.evaluate(async (url) => {
    ...
    gltf.scene.traverse((object) => {
      ...
      if (
        object.userData?.schema === "bf3d.physical_maintenance_access.v1" &&
        object.userData?.semanticKind === "physical_maintenance_access_system"
      ) {
        result.maintenanceAccessSystemCount += 1;
      }
      if (object.userData?.semanticKind === "physical_maintenance_platform") {
        result.maintenancePlatformCount += 1;
        result.platformIds.push(object.userData.platformId);
      }
      if (object.userData?.semanticKind === "physical_annular_walkway_deck") {
        result.walkwayDeckCount += 1;
      }
      if (object.userData?.semanticKind === "physical_double_guardrail_ring") {
        result.guardrailRingCount += 1;
      }
      if (object.userData?.semanticKind === "physical_guardrail_posts") {
        result.guardrailPostGroupCount += 1;
        result.guardrailPostInstanceCount += Number(object.userData.instanceCount || object.count || 0);
      }
      if (object.userData?.semanticKind === "physical_radial_support_brackets") {
        result.radialBracketGroupCount += 1;
        result.radialBracketInstanceCount += Number(object.userData.instanceCount || object.count || 0);
      }
      if (object.userData?.semanticKind === "physical_diagonal_support_braces") {
        result.diagonalBraceGroupCount += 1;
        result.diagonalBraceInstanceCount += Number(object.userData.instanceCount || object.count || 0);
      }
      if (object.userData?.semanticKind === "physical_connecting_access_ladder") {
        result.accessLadderCount += 1;
      }
      if (object.userData?.semanticKind === "physical_access_ladder_side_rail") {
        result.accessLadderSideRailCount += 1;
      }
      if (object.userData?.semanticKind === "physical_access_ladder_rungs") {
        result.accessLadderRungGroupCount += 1;
        result.accessLadderRungInstanceCount += Number(object.userData.instanceCount || object.count || 0);
      }
  ...
  const passed =
    ...
    pageErrors.length === 0;
  const report = {
    schema: "bf3d.glb_img2threejs_maintenance_access_reopen_check.v1",
