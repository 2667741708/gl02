...
DiagnosisSummary=function BFDiagnosisSummaryNormalGreen({diagnosis}){const d=getDiag(diagnosis),rec=getRec(diagnosis),ev=(d.evidence.length?d.evidence.map(e=>e.text||String(e)):(RULE_KNOWLEDGE[d.label]?.evidence||[]).slice(0,3)).map(typeof workerText==='function'?workerText:(x=>x));const mainCls=diagnosisToneClass(d.label,d.score),secondaryCls=secondaryDiagnosisToneClass(d.secondary);return <div className="diagnosis-hero bf-diagnosis-normal-green"><div><div className="diag-line">主诊断：<b className={mainCls}>{LABELS[d.label]}</b></div><div className="diag-line">次诊断：<b className={`secondary ${secondaryCls}`}>{d.secondary?LABELS[d.secondary]:'暂无明显并发'}</b></div></div><div className="confidence"><span>判断把握：{fmtNum(d.score,0)}/100</span><div className="bar"><div className="bar-fill" style={{width:`${Math.min(100,Math.max(0,d.score))}%`}}></div></div><span>{fmtNum(d.score,0)}%</span></div><div className="evidence-list">{ev.slice(0,4).map((x,i)=><div className="evidence-item" key={i}><b>依据 {i+1}</b>：{x}</div>)}<div className="evidence-item"><b>目标</b>：{typeof workerText==='function'?workerText(rec.goal):rec.goal}</div></div></div>}
DiagnosisConclusion=function BFDiagnosisConclusionNormalGreen({diagnosis}){const d=getDiag(diagnosis),score=Math.max(0,Math.min(100,Number(d.score)||0)),mainCls=diagnosisToneClass(d.label,d.score),secondaryCls=secondaryDiagnosisToneClass(d.secondary);return <div className="diag-conclusion bf-diagnosis-normal-green"><MiniFurnaceIcon/><div className="diag-conclusion-lines"><div className="diag-line">主诊断：<b className={mainCls}>{LABELS[d.label]}</b></div><div className="diag-line">次诊断：<b className={`secondary ${secondaryCls}`}>{d.secondary?LABELS[d.secondary]:'暂无明显并发'}</b></div><div className="diag-line" style={{fontSize:22}}>判断把握：<b style={{color:'#fff'}}>{LABELS[d.label]} {fmtNum(score,0)}/100</b></div></div></div>}
