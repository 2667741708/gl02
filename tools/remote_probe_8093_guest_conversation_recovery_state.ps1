...
$OutputPath = 'C:\Users\Administrator\AppData\Local\Temp\bf_8093_guest_conversation_recovery_20260814\remote-state.json'
...
$Branch = (& $GitExe -C $Root branch --show-current 2>$null | Out-String).Trim()
...
$State = [ordered]@{
    ...
    targets = [ordered]@{
        $Target = [ordered]@{ exists = $true; sha256 = $TargetHash }
    }
    production = [ordered]@{
        git_head = $Head
        git_branch = $Branch
