...
用户限定同一时间只能有一个人使用。8093访客、操作员、管理员与不同会话共用一个生成名额；其他发送返回409使用中，不排队、不自动重放。取消待实际停止才释放，失败在finally释放；查看页面与历史不占用。本机实现、35项关联测试及独立审查通过，生产尚未更新。范围为单8093进程，不含8094或整个登录会话锁。[实施与待发布验收](handoffs/2026-09-16-qa-exclusive-use-local.md)。
