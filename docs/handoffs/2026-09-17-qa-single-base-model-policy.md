...
状态：固定身份本机实现及候选已验证；禁切换管理器尚未安装，生产身份仍漂移。当前路由候选以[V49追问执行交接](2026-09-17-qa-owned-followup-execution.md)为准，共享生产变更保留见[V47集成](2026-09-17-qa-shared-proxy-integration.md)；下文V27/V26记录为历史阶段证据。
...
### 22:26 独立根因调查与 Qwen3.8 来源核验

[独立只读报告](2026-09-17-model-identity-switch-independent-audit.md)核对了恢复任务 PT1M 触发、真实 Repair 进程、远端管理器行号和对应事件：旧策略先把 :1 复制到 latest，任何后续失败又 fallback 到 :0，失败时不更新期望状态，下一轮继续从 :1 开始。这条闭环支持频繁切换的主要解释；并未取得每次 copy 的系统级 PID 归属追踪。两次预热服务端均200，具体后续HTTP或响应处理失败阶段仍未核实，不能直接称为预热失败或OOM。

[权重来源核验](2026-09-17-locked-qwen38-lineage-verification.md)实际读取冻结31629f… GGUF 的 general.name=Qwen3.8-27B，并核对固定上游 revision 的公布摘要；qwen35 是兼容架构标记，不能据此将冻结底座判为 Qwen3.5。来源支持固定Qwen3.8量化产物导入，不证明另有高炉业务微调。

本轮五组固定身份/管理器/旧窗口/batch/安装事务回归61 passed、21.83秒。生产仍为旧管理器856f…，禁切换安装未执行、永久固定未验收；本轮独立调查及主任务未加载、切换或删除模型，未发送业务问答POST。实际复现命令与唯一已用basetemp见来源核验报告。

## 候选与保留边界

### V49后的最新独立快照

UTC2026-09-17 12:58:09 tags→ps→tags均匹配冻结e4；12:58:40独立复核中，两次tags及全部ps记录又变为同名9111。两个时点分别保留，不将瞬时一致声称为永久锁定，也不据此认定执行者。固定模块及pin不变，禁止自动切回、fallback、同名换权重及重新选基线。0主任务换模/模型调用/原题POST/生产写。905本机回归、40原生合成合同通过只证明候选，生产部署及822原题复测仍阻断；[脱敏证据](../../tests/qa_regression/owned_followup_execution_20260917.json)。

### V48后的最新独立快照

UTC2026-09-17 12:27:33，Reliable SSH tags→ps→tags独立只读检查中，公共别名与唯一驻留记录均为`9111be…`，不符合冻结`e4ad74…`。固定底座候选模块逐字节继承，未改变pin，0真实模型调用/原题POST/模型或任务修改。861项本机回归与25原生合成合同通过不证明真实生产身份或原题准确率；生产部署及原题复测继续阻断。不能自动切回、另立基线或使用备用模型。
...
## 本轮模型身份恢复器修复：当前候选与历史快照

本轮按[完整子智能体修复提示词](2026-09-17-model-identity-repair-agent-prompt.md)执行，根因证据见[独立调查](2026-09-17-model-identity-switch-independent-audit.md)。最终候选、真实测试及控制流由[本轮实施报告](2026-09-17-model-identity-repair-implementation.md)记录；上文r1管理器哈希、52项测试及最初候选JSON属于历史快照，不能作为当前安装密封。

当前恢复器将identity_ready、dependencies_ready、observability_ok、assistant_ready分别记录；只允许同一冻结底座的别名修复和空驻留重载，健康驻留不预热，错误/多驻留阻断。状态先原子写pending false，执行中间观测后才最终原子提交；取消、原生PipelineStopped或最终写失败不能留下新写入的助手就绪状态。已观察过的摘要在下一次读取前清空，读取失败用null，不填旧值冒充本次实测。

本机修复及独立审查不证明生产生效。本轮没有上传、安装、模型调用、任务停启或原题POST；生产旧恢复器仍待单独受控安装，822原题优化后准确率尚无新增证据。

最终r4候选SHA-256为`1f871a8bbbaa553233d318b3d019a5a63e68be38d0f0caa7d29cca731d44434f`；本机安装器SHA-256为`810a6409c62f26298f2954b32c98380d2dc4e6e0a5c3a98309380c9e35b6f38f`。独立gpt-5.6-luna只读审查已对两份密封给出PASS；实际受影响集合87个不同用例通过、关联53个不同用例通过，共140个本机检查通过。分组命令、JUnit终止证据和历史失败见本轮实施报告，不作为生产问题数或回答成功率。
