...
定时复测的密封计划固定 `Asia/Shanghai`、22:30–07:30、10 秒观察间隔、3 次稳定观察、真实用户静默 300 秒和 GPU 上限 5%。这些参数只控制测试 worker 是否 claim 下一题，不写入 8093 服务配置、模型配置或数据库。worker 缺少 GPU 状态、固定模型、V52 Git 祖先、点位目录或任一运行 pin 时均 fail-closed。
