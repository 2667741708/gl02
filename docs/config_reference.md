...
V52 不新增环境变量，不调整 MCP 轮数、超时、并发、知识模式或模型配置。固定模型名称、digest、禁止切换、禁止回退和禁止同名权重替换继续继承 V51。点位目录行为由 TaskPlan 和已注册只读工具白名单决定；新增目录工具必须同时更新白名单与回归，不能只靠工具名推断安全性。

## V52 定时复测运行参数（2026-09-21）

定时复测的密封计划固定 `Asia/Shanghai`、22:30–07:30、10 秒观察间隔、3 次稳定观察、真实用户静默 300 秒和 GPU 上限 5%。这些参数只控制测试 worker 是否 claim 下一题，不写入 8093 服务配置、模型配置或数据库。worker 缺少 GPU 状态、固定模型、V52 Git 祖先、当前生产提交、8093 PID/创建时间、点位目录或任一运行 pin 时均 fail-closed。当前 r6 对 78 个运行文件做精确 pin。

运行文件首次读到非预期 hash 时最多再读 5 次；只有连续 3 次恢复为预期 hash 才把该次视为瞬态读取，持续或反复异常仍停止且不 claim。密封 stage 自身的 manifest 和文件 hash 不使用该容错，任何不一致立即停止。

远端 stage、输出、manifest/plan SHA 和停止/恢复边界见[定时复测交接](handoffs/2026-09-21-qa-v52-scheduled-failure-retest.md)。
